...
class FlatPagesBase(ABC):
    """
    Abstract Class for a FlatPages backend

    FlatPages are a collection of :class:`Page` objects.
    ...
    @abstractmethod
    def _load_page(self, url_path: str, page_id: Any) -> Page: ...

    @abstractmethod
    def _cache_page(self, url_path: str, page_id: Any, page: Page): ...

    @abstractmethod
    def _retieve_page(self, url_path: str) -> tuple[Any, Page]: ...

    def __iter__(self) -> Iterator[Page]:
        """Iterate on all :class:`Page` objects."""
        for page_path, page_data in self._walk_pages():
    ...
    def get(
        ...
        """
        try:
            page_id, page = self._retieve_page(path)
            if reload:
    ...
    def get_or_404(self, url_path: str) -> Page:
        """
        Return the :class:`Page` object at ``url_path``.
    ...
    def register_page(self, page_id: Any, url_path: str):
        """Add a new page without rescanning the rest

        This method exists to allow quickly registering new pages.

        .. versionadded: 1.0
        """
        self._cache_page(url_path, page_id, self._load_page(url_path, page_id))

    def reload_page(self, url_path: str) -> Page:
        """Reload a single page, without triggering a full rescan.

        This method allows fr quickly reloading new pages when serving them

        .. versionadded: 1.0
        """
        page_id, _ = self._retieve_page(url_path)
        page = self._load_page(url_path, page_id)
        self._cache_page(url_path, page_id, page)
        return page
...
class FlatPages(FlatPagesBase):
    """A collection of :class:`Page` objects, served from the filesystem."""
    ...
    @cached_property
    def _pages(self) -> dict[str, tuple[str, Page]]:
        """
        Build and return a dict of pages

        Returns a dictionary of pages and their filesystem path, keyed by their
        url path.
        """
        pages = {}
    ...
    def _cache_page(self, url_path: str, page_id: Any, page: Page):
        self._pages[url_path] = (page_id, page)

    def _retieve_page(self, url_path: str) -> tuple[str, Page]:
        return self._pages[url_path]
    ...
    def _walk_pages(self) -> Generator[tuple[str, str], None, None]:
        """
        Walk the filesystem and yield valid pages

        This method takes into account the configured file extensions,
        """
    ...
    def _load_page(self, url_path: str, page_id: str) -> Page:
        """
        ...
        if cached and cached[1] == mtime:
            ...
            page = self._parse(content, url_path)
            self._file_cache[filename] = (page, mtime)

        return page

    def _parse(self, content: str, url_path: str) -> Page:
        ...
        if self.config("legacy_meta_parser"):
            meta, content = legacy_parser(content, url_path)
        else:
            meta, content = libyaml_parser(content, url_path)
        ...
        try:
            folder, _ = url_path.rsplit("/", maxsplit=1)
        except ValueError:
            folder = ""

        # Initialize and return Page instance
        return Page(url_path, meta, content, html_renderer, folder)
